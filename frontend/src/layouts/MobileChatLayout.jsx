...
function MobileChatLayout() {
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden" style={{ background: "var(--surface)" }}>
      {selectedUser ? (
        <div
          key="chat"
          className="flex-1 flex flex-col z-50 h-full absolute inset-0"
        ...
        >
          {/* Top header for list */}
            <div className="px-5 pt-6 pb-3 flex items-center justify-between">
              <h2 style={{ fontSize: "1.25rem", fontWeight: 700, color: "var(--on-surface)" }} className="capitalize">{currentTab}</h2>
            </div>
            ...
            {/* Bottom Nav */}
            <div className="flex justify-around items-center px-4 py-3 border-t bg-[var(--surface-low)]" style={{ borderColor: "var(--surface-high)" }}>
               {navItems.map(item => {
